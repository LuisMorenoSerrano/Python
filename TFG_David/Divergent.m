...
global gama   = 1.4;     % Air Property
global ncl    = 10;      % Number of Characteristics Lines
global M_exit = 2.4;
global D_exit = 0.4811;  % Change This Value to Achieve Exit Diameter
...
%% Exit Conditions
PM_exit  = (sqrt((gama + 1) / (gama - 1)) * ...
           atan(sqrt((gama - 1) / (gama + 1) * (M_exit^2 - 1)))) - ...
           atan(sqrt(M_exit^2 - 1));                       % Radian
...
for i = 1 :nnode
  if i < wall(j + 1)
    if wall(j) == 0  % Interactions With Point A
      ...
      end  % Finish of A point Interactions
      ...
      a_minus = (teta(i-l, 1) + teta(i + 1, 1)) / 2 - ...
                (mu(i-l, 1) + mu(i + 1, 1)) / 2;
      m_C_minus(i + 1, 1) = tand(a_minus);
  ...
  elseif  i == wall(j + 1)  % Wall Points
    ...
    x(i + 1, 1) = (y(wall(j) + 1) - y(i) + m_C_plus(i + 1, 1) ...
                    * x(i) - m_C_wall(i + 1, 1) * x(wall(j) + 1)) / ...
                  (m_C_plus(i + 1, 1) - m_C_wall(i + 1)) ;
